...
The test suite is 304 unit tests at 89% coverage and 114 integration tests against the real Anthropic API — including 20 prompt injection attacks, 19 sensitive-topic escalations, and a full ask-escalate-fix-reask-cite cycle. CI runs 13 automated checks per PR: typecheck, lint, SAST (Semgrep + Bearer), secrets scanning (TruffleHog), dependency audit, container image scanning (Trivy), and Claude Code review with 7 specialized agents.
